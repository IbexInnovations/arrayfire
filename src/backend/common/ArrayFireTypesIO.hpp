...
struct fmt::formatter<arrayfire::common::Version> {
    ...
    auto format(const arrayfire::common::Version& ver, FormatContext& ctx) const
        ...
        bool can_show_minor = show_minor;
        bool can_show_patch = show_patch;
        if (ver.minor() == -1) can_show_minor = false;
        if (ver.patch() == -1) can_show_patch = false;
        if (show_major && !can_show_minor && !can_show_patch) {
